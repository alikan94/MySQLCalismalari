...
select isim, sum(urun_miktari) as toplam_miktar 
from manav 
group by isim;
...
from manav 
where urun_adi is not null -- null almadan
group by urun_adi -- urun adina göre gruplandırdık
order by max desc; -- maxa göre azalan sıraladık
