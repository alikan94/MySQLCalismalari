...
select marka_isim, calisan_sayisi, (select sum(maas) from calisanlar where marka_isim = isyeri) as toplam_mass from markalar;
-- as yazarak kolona isim atayabiliriz
...
-- round() ile yuvarlama yapabiliriz, round(x,2) 2 virgülden sonra 2 basamak ekler
...
select marka_id, marka_isim, (select count(sehir) from calisanlar where isyeri = marka_isim) as toplam_sehir from markalar;
