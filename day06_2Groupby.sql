...
select ulke, count(id) as calisan_sayisi
from personel 
group by ulke;
...
select ulke, sehir, count(*) as personel_sayisi
from personel
group by ulke, sehir;
/* hocanin cozumu
...
select ulke, cinsiyet, count(*) as calisan_sayisi, round(avg(yas)) as ortalama_yas
from personel
group by cinsiyet, ulke
order by ulke, cinsiyet;
-- hocanın cozumu
...
select ulke, cinsiyet, count(*) as calisan_sayisi, round(avg(yas)) as ortalama_yas, maas
from personel
where maas > 30000
group by ulke, cinsiyet
order by ulke, yas desc;
-- hocanın cozumu
...
select isim, urun_adi, sum(urun_miktari) as toplam_urun
from manav
-- where toplam_urun >= 3 -- hata verdi
group by isim, urun_adi
...
select urun_adi, max(urun_miktari) as max
from manav
group by urun_adi
having max <> min(urun_miktari) -- <> bunun yerine != kullanılabilir
order by max;
-- hocanin cozumu
...
select count(distinct urun_adi) as meyve_turu_sayisi from manav;
